...
class DrivingGrid(SimpleDrivingEnv):
    ...
    def __init__(self):
        self.action_space = gym.spaces.box.Box(
            low=np.array([0, -0.6, 0], dtype=np.float32),
            high=np.array([1, 0.6, 1], dtype=np.float32),
        )
    ...
    def grid2meter(self):
        return (2 * self.max_half_length) / (self.grid.shape[0]), (
            2 * self.max_half_length
        ) / (self.grid.shape[1])
