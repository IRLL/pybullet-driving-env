...
class SimpleDrivingEnv(gym.Env):
    ...
    def __init__(self):
        self.action_space = gym.spaces.box.Box(
            low=np.array([0, -0.6], dtype=np.float32),
        ...
        """
        # TODO: For some reason the return images is 75x75x2
        obs_space = {
            "car_qpos": gym.spaces.box.Box(
                low=np.array(
                    [-np.Inf, -np.Inf, -1, -1, -1, -np.Inf, -np.Inf],
                    dtype=np.float32,
                ),
                high=np.array(
                    [np.Inf, np.Inf, 1, 1, 1, np.Inf, np.Inf], dtype=np.float32
                ),
            ),
            "segmentation": gym.spaces.box.Box(
                low=np.zeros((75, 75, 2)),
                high=np.ones((75, 75, 2)),
                shape=(75, 75, 2),
            ),
        }
        ...
        self.projectionMatrix = p.computeProjectionMatrixFOV(
            fov=50.0, aspect=1.0, nearVal=0.1, farVal=100.5
        )
    ...
    def step(self, action, max_step_multiplier=1, agent="alice"):
        ...
        return (
            {"segmentation": np.float32(gridmap), "car_qpos": ob},
            reward,
            self.done,
            dict(),
        )
    ...
    def reset(self, goal, base_position, base_orientation, agent="alice"):
        ...
        return {
            "segmentation": np.float32(gridmap),
            "car_qpos": np.array(car_ob, dtype=np.float32),
        }
    ...
    def reset_obstacle_positions(self):
        ...
        for i in range(2):
            ...
            if r == 0:
                self.pos_obstacles[i][0:2] = mult * (
                    np.random.uniform(0, 5, 2) + np.array([0, 5])
                )
            elif r == 1:
                self.pos_obstacles[i][0:2] = mult * (
                    np.random.uniform(0, 5, 2) + np.array([5, 5])
                )
            elif r == 2:
                self.pos_obstacles[i][0:2] = mult * (
                    np.random.uniform(0, 5, 2) + np.array([5, 0])
                )
        mult = -1
        for i in range(2):
            mult = -1 * mult
            r = np.random.randint(0, 3)
            if r == 0:
                self.pos_obstacles[i + 2][0:2] = mult * (
                    np.random.uniform(0, 5, 2) + np.array([-5, 5])
                )
            elif r == 1:
                self.pos_obstacles[i + 2][0:2] = mult * (
                    np.random.uniform(0, 5, 2) + np.array([-10, 5])
                )
            elif r == 2:
                self.pos_obstacles[i + 2][0:2] = mult * (
                    np.random.uniform(0, 5, 2) + np.array([-10, 0])
                )
